...
`ngHandsontable.js` changed to support reassigning `datarows` and/or `columns`, for which it will recreate the entire table. `index.html` and `ngHandsontableDemo.js` modified to isolate the "dynamic columns" example, and shuffles the rows and columns to demo `datarow` and `column` reassignment.
